...
- **소속 공공기관** — 부처·처·청·위원회별 공공기관 연결을 공식 지정현황 기준으로 보강, 상세 패널 정보 밀도 강화
- **대표 정책지표** — e-나라지표/KOSIS/부처 통계 기반 핵심 지표 64개를 19개 부처 상세 패널에 연결
- **근거 메타데이터 강화** — 정책지표별 dataset id, publisher, last verified, update cadence, coverage, methodology를 source catalog와 함께 표시
...
- **상세 패널** — 데스크탑: 우측 사이드 패널 / 모바일: 하단 바텀시트 (스와이프 확장), 소관 업무→기관장→내부 조직→산하기관→공공기관→정책지표→출처→Data Backbone 순으로 정보 재배치
- **딥링크 & 공유** — URL 해시(`#국방부`)로 특정 기관 바로 열기, 상세 패널에서 공유 버튼으로 URL 복사
- **공유용 URL 파라미터** — `?view=org|radial`, `?budget=1`, `?theme=dark|light`로 특정 화면을 즉시 공유 가능. 해시와 결합 가능 (예: `?view=org&budget=1#국방부`). localStorage를 건드리지 않아 방문자의 저장된 테마를 덮어쓰지 않음
...
이 구조 덕분에 정책지표·추가 공공데이터를 UI 노드 이름에 직접 매달리지 않고 `canonicalId` 중심으로 조인할 수 있다. 이번 패스에서는 정책지표 source를 `data/policy-indicators/*.json`으로 분리하고, `sync-policy-indicators.mjs`로 UI 번들에 동기화하도록 정리했다. 현재는 19개 전체 부처를 커버하는 64개 지표를 연결했고, 각 지표 fact에는 `series`와 선택적 `target` 메타를 함께 실어 추이·YoY·기간 최고/최저·목표 배지 UI의 기반을 마련했다. 추가로 source catalog에 `datasetId`, `publisher`, `lastVerified`, `updateCadence`, `coverage`, `methodology`, `qualityNote`를 붙이고, indicator bundle로 자동 상속되게 만들어 정책카드와 출처 모달에서 근거 강도를 더 명시적으로 볼 수 있게 했다.
